          ...
          >
            <div>
              <div className="flex justify-between items-start mb-2.5">
                <span className="font-label-caps text-label-caps bg-[#0C133D] text-[#D4AF37] px-2.5 py-1 uppercase rounded-md text-[11px] font-bold">
                ...
                <span className="font-data-tabular text-xs text-on-surface-variant">{item.date}</span>
              </div>
              <h3 className="font-headline-md text-lg sm:text-xl font-bold text-[#0C133D] hover:text-[#D4AF37] transition-colors mb-2 leading-snug line-clamp-2">
                {item.title}
              </h3>
            </div>
            <p className="font-body-md text-xs sm:text-sm text-on-surface-variant leading-relaxed line-clamp-2 mt-auto">{item.summary}</p>
          </Reveal>
        ))}
      </div>
    </section>
