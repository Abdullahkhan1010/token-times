...
export default function Footer() {
  return (
    <footer className="bg-[#0C133D] text-white w-full py-10 px-4 md:px-12 flex flex-col md:flex-row justify-between items-start border-t border-white/10">
      <div className="flex flex-col mb-8 md:mb-0 max-w-sm space-y-3">
        <h2 className="font-headline-lg text-2xl font-bold text-white">Tokens Times</h2>
      ...
      <div className="flex flex-wrap gap-x-12 gap-y-6">
        <div className="flex flex-col gap-3">
          {["About Us", "Contact", "Privacy Policy", "Terms of Service"].map((t) => (
            <a key={t} className="font-label-caps text-xs text-white/80 hover:text-[#D4AF37] transition-colors" href="#">
              {t}
            </a>
          ))}
        </div>
