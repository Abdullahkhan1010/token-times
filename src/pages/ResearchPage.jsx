    ...
    <div className="space-y-10">
      ...
      <Reveal as="div" className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {keyMetrics.map((m) => (
          <div key={m.label} className="bg-surface-container-lowest border border-outline-variant rounded-xl p-5 text-center space-y-1 shadow-sm">
            <span className="font-display-lg text-2xl sm:text-3xl font-extrabold text-accent block">{m.value}</span>
      ...
      >
        <div className="lg:col-span-5 relative h-64 sm:h-80 lg:h-full rounded-xl overflow-hidden">
          ...
          <span className="absolute top-3 left-3 bg-accent text-on-accent text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">
        ...
        <div className="lg:col-span-7 flex flex-col justify-between space-y-4">
          <div className="space-y-3">
            <span className="font-label-caps text-xs text-accent font-bold tracking-wider uppercase">FLAGSHIP PUBLICATION</span>
            <h2 className="font-headline-lg text-2xl sm:text-3xl font-bold text-on-surface leading-tight">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-outline-variant/40">
            ...
            <button className="px-5 py-2.5 bg-accent text-on-accent font-bold text-xs rounded-xl hover:bg-accent-dark transition-colors flex items-center gap-2 shadow">
      ...
      <div className="space-y-4">
        <h2 className="font-headline-sm text-xl font-bold text-primary border-b border-outline-variant pb-2 flex items-center gap-2">
          <BookOpen size={20} className="text-accent" /> Published Technical Papers & Macro Audits
        </h2>
          ...
          {papers.map((p, i) => (
            ...
            >
              <div className="space-y-2">
                <span className="text-[10px] font-bold text-accent uppercase tracking-wider block font-data-tabular">
                  {p.meta}
                </span>
